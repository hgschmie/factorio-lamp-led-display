...
/* [What to render] */
part = "assembly"; // [base, lid, board, assembly, exploded]
...
/* [Sliding lid] */
groove_depth   = 1.4;  // how far the groove cuts into the side walls
groove_clear   = 0.3;  // vertical slack in the groove. TUNE
lid_side_clear = 0.25; // horizontal slack per side. TUNE
top_lip        = 1.5;  // wall material above the groove
...
groove_z = floor_t + cavity_h;                  // groove floor = lid underside
groove_h = lid_t + groove_clear;
case_h   = groove_z + groove_h + top_lip;       // overall base height
board_off = [wall + board_clear + usb_gap, wall + board_clear];

lid_w = inner[0] + 2 * groove_depth - 2 * lid_side_clear;
lid_l = inner[1] + groove_depth - lid_side_clear + wall; // front groove .. flush with back face
lid_block_h = case_h - (groove_z + lid_t);      // rear block fills the recess above the lid
...
echo(str("outer=", outer, " case_h=", case_h, " groove_z=", groove_z, " lid=", [lid_w, lid_l],
...
module base() {
    difference() {
        ...
        // lid groove: left + right walls and the front wall (end stop), open at the back
        translate([wall - groove_depth, wall - groove_depth, groove_z])
            cube([inner[0] + 2 * groove_depth, inner[1] + groove_depth + wall + eps, groove_h]);
        // back wall lowered to the groove floor so the lid can slide in
...
// ---------------------------------------------------------------- lid
module lid() {
    y0 = wall - groove_depth + lid_side_clear;  // front edge sits in the front groove
    difference() {
        union() {
            translate([wall - groove_depth + lid_side_clear, y0, groove_z])
                cube([lid_w, lid_l, lid_t]);
            // rear block: fills the recess above the lid, flush with the wall tops, acts as a grip
            translate([wall + lid_side_clear, outer[1] - wall, groove_z + lid_t])
                cube([inner[0] - 2 * lid_side_clear, wall, lid_block_h]);
        }
